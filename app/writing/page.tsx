...
export default function WritingPage() {
  ...
  return (
    <main className="max-w-2xl mx-auto px-6">
      ...
      <div className="mb-12">
        <h1 className="text-xl font-medium text-stone-900 mb-3">Writing</h1>
        <p className="text-sm text-stone-500 leading-relaxed">
          Personal perspectives — mostly on relationships and intimacy. Honest attempts to think
          clearly about things that resist clear thinking.
        </p>
      </div>
      ...
      {posts.length === 0 ? (
        ...
        <ul className="space-y-8">
          {posts.map(post => (
            <li key={post.slug}>
              <Link href={`/writing/${post.slug}`} className="group flex gap-6 items-baseline">
